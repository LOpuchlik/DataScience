...
hidden_nodes = 10 # number of neurons in the hidden layer
learning_rate = 0.001
...
for epoch in range(1, 50):
    Z = W1 @ X_train + B1 @ np.ones(X_train.shape)
    ...
    dW1 = learning_rate * (np.exp(Z) * (np.exp(Z)+1)) * E1 @ np.transpose(X_train)
    dB1 = learning_rate * (np.exp(Z) * (np.exp(Z)+1)) * E1 @ np.transpose(np.ones(X_train.shape))
    ...
    if epoch % 1 == 0:
        ...
        plt.scatter(X_train, A2.T, c='r', label='model') 
        plt.xlim([-1, 6])
        plt.ylim([-1, 1])
        plt.legend()
        fig.canvas.draw()
